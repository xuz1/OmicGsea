...
package: OmicGsea
abstract: >
  A brief introduction of OmicGsea R package for gene set enrichment analysis.
vignette: >
  %\VignetteIndexEntry{OmicGsea User's Guide}
...
poses challenges for gene set enrichment analysis. The `r Biocpkg("OmicGsea")` package provides both 
...
including DNA methylation, proteomics, genotyping, and genome sequencing data. The package also 
includes a fast and accurate ranking-based enrichment method specifically tailored for gene expression
and RNA-seq data.
...
gseaGene(probe.p=probe.p,Data4Cor=Data4cor,arrayType="450K",nTopGene=1000,outGenep=TRUE,
	 method="Threshold",gSetName="KEGG",species="Human",outfile="gsea1",ncore=50)

#for significant genes at FDR threshold of 0.05
gseaGene(probe.p=probe.p,Data4Cor=Data4cor,arrayType="450K",FDRthre=0.05,
         method="Threshold",gSetName="KEGG",species="Human",outfile="gsea2",ncore=50)

#geneset enrichment analysis with ranking based method
gseaGene(probe.p=probe.p,Data4Cor=Data4cor,arrayType="450K",gseaParam=1,
         method="Ranking",gSetName="KEGG",species="Human",outfile="gsea3",ncore=50)

#without using methylation data matrix for correlation
gseaGene(probe.p=probe.p,arrayType="450K",nTopGene=1000,
         method="Threshold",gSetName="KEGG",species="Human",outfile="gsea4",ncore=50)

#Enrichment analysis with probe group level P values
gseaPG(probe.p=probe.p,Data4Cor=Data4cor,arrayType="450K",nTopPG=1000,
        gSetName="KEGG",species="Human",outfile="gsea1_2",ncore=50)
...
gseaProbe(probe.p=probe.p,nTopProbe=1000,arrayType="450K",gSetName="KEGG",
	  species="Human",ncore=50)
```

## Example 2: Enrichment analysis for arrays other than methylation 450K and EPIC

```{r example2, eval=TRUE,results = "hide"}
...
#enrichment analysis
gseaGene(probe.p=probe.p,Data4Cor=dat,GeneProbeTable=GeneProbeTable,
         method="Threshold",gSetName="KEGG",species="Human",outfile="gsea5",ncore=50)
gseaGene(probe.p=probe.p,Data4Cor=dat,GeneProbeTable=GeneProbeTable,
         method="Ranking",gSetName="KEGG",species="Human",outfile="gsea6",ncore=50)
```

## Example 3: Enrichment analysis with user provided geneset

```{r example3, eval=TRUE,results = "hide"}
#generatin example dataset
userGeneset=getKEGG(species="Human")

#enrichment analysis
gseaGene(probe.p=probe.p,Data4Cor=dat,GeneProbeTable=GeneProbeTable,
         method="Threshold",geneSet=userGeneset,species="Human",outfile="gsea7",ncore=50)
gseaGene(probe.p=probe.p,Data4Cor=dat,GeneProbeTable=GeneProbeTable,
         method="Ranking",geneSet=userGeneset,species="Human",outfile="gsea8",ncore=50)
```

## Example 4: Enrichment analysis with user provided geneset


```{r example4, eval=TRUE,results = "hide"}
#generatin example dataset
userGeneset=getKEGG(species="Human")
...
```{r example5, eval=TRUE,results = "hide"}
#generatin example dataset
kegg=getKEGG(species="Human")
...
gseaRank(stats=stats,gSetName="KEGG",scoreType="std",outfile="gsea9",nperm=1e5,ncore=50)
#enrichment of genes with higher statistics
stats=sort(abs(stats),decr=TRUE)
gseaRank(stats=stats,gSetName="KEGG",scoreType="std",outfile="gsea10",nperm=1e5,ncore=50)
...
gseaGene(probe.p=probe.p,nTopGene=500,GeneProbeTable=GeneProbeTable,gSetName="KEGG",outfile="gsea11",ncore=5)
gseaProbe(probe.p=probe.p,nTopProbe=500,GeneProbeTable=GeneProbeTable,gSetName="KEGG",outfile="gsea12",ncore=5)
```
