...
#main function
gseaGene<-function(probe.p,Data4Cor=NULL,method="Threshold",FDRthre=0.05,nTopGene=NULL,
		   GeneProbeTable=NULL,arrayType=NULL,gSetName="KEGG",geneSet=NULL,
		   species="Human",combpMethod="fisher",combpAdjust = "nyholt",
		   outfile="gseaGene",outGenep=FALSE,gseaParam=1,nperm=1e4,ncore=5){
...
if(outGenep){
        ...
        write.csv(tmp,"gseaGene_genep.csv",row.names=FALSE)
}
...
if(method=="Ranking"){
	...
	if(length(sig.gene)<10){stop("Number of significant gene is less than 10, 
				     please adjust threshold or use ranking method in gseaGene()")}
